...
async def test_fetch_mentions_bird_success():
    ...
    with patch("app.integrations.bird._run_bird", new_callable=AsyncMock, return_value=mock_data):
        from app.integrations.bird import fetch_mentions_bird
        result = await fetch_mentions_bird("sawinyh", count=10)
...
async def test_fetch_mentions_bird_strips_at_prefix():
    ...
    with patch("app.integrations.bird._run_bird", new_callable=AsyncMock, return_value=mock_data) as mock_run:
        from app.integrations.bird import fetch_mentions_bird
        result = await fetch_mentions_bird("@sawinyh", count=5)
...
async def test_fetch_user_replies_bird_filters_to_replies():
    ...
    with patch("app.integrations.bird._run_bird", new_callable=AsyncMock, return_value=mock_data):
        ...
        result = await fetch_user_replies_bird("sawinyh")
