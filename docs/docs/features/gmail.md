...
RealCRM connects to Gmail via OAuth 2.0 with Google, syncing email threads, contacts, and calendar events to build a complete interaction history.

## Authentication

The Gmail integration uses standard Google OAuth 2.0. After granting access on the Google consent screen, RealCRM stores a refresh token to maintain access without repeated sign-in. Multi-account support allows connecting more than one Gmail address.
...
A one-way import pulls contacts from Google Contacts into RealCRM. Contact records include names, email addresses, phone numbers, and organization fields. This import does not write back to Google -- it is read-only.
